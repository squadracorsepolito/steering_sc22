...
/*
Per calcolare il checksum, devo effettuare lo xor di coppie di bytes, per cui, se ad esempio ho "0a24", dovr� fare lo xor tra 0a e 24
...
bool WriteStr (uint8_t cmd, uint8_t index, uint8_t length, uint8_t message[], uint8_t checksum){
	...
	// Con le stringhe, trasmetto una variabile alla volta, perch� altrimenti non funziona con la trasmissione del messaggio di tipo stringa
	...
	HAL_UART_Transmit(&huart1, &message[i], 1, 100);	// ciclo su tutta la lunghezza della stringa, cos� da inviare un valore per volta
	HAL_Delay(1);
	}
...
void screen(uint8_t RTD, uint8_t message[], size_t len, uint32_t delay_100us) {
	...
	switch (RTD){
		...
		case 0: 
			...
			WriteObject(WRITEOBJ, FORM, SCREEN0, MSB_FORM0, LSB_FORM0, checksum);	// 0x0B � il checksum per la schermata main
		...
		WriteObject(WRITEOBJ, FORM, SCREEN1, MSB_FORM0, LSB_FORM0, checksum);	// 0x0A � il checksum per la schermata RTD
		delay_fun(&delay_100us_last, delay_100us);	// piccolissimo ritardo per iniziare a cambiare i valori sul display
...
void check_push_dx(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin, uint32_t delay_100us, uint8_t screen){
	...
	if (HAL_GPIO_ReadPin(GPIOx, GPIO_Pin) == 1)		{	// Push 1: incrementa
		...
		if (waiting){
			if (delay_fun(&time, delay_100us))	// se � passato un tempo maggiore di time	
					{
						if (HAL_GPIO_ReadPin(GPIOx, GPIO_Pin) == 1){		// ri-checko che il pulsante sia ancora pushato
							...
							if (delay_fun(&MAX_DELAY, delay_100us) && HAL_GPIO_ReadPin(GPIOx, GPIO_Pin)==0){	// se passano 10 secondi e non sto pi� premendo il pulsante, torno alla schermata RTD
							checksum = checksum_objects(WRITEOBJ, FORM, SCREEN1, MSB_FORM0, LSB_FORM0);	
							WriteObject(WRITEOBJ, FORM, SCREEN1, MSB_FORM0, LSB_FORM0, checksum);	// se dopo 10 secondi non premo pi� nulla, torno al form1 RTD
							page = 0;
							}
						
						waiting = 0;	// non sono pi� in attesa, quindi lo metto a zero
						first_push = 1;	// rimetto il push a 1, pronto per la prossima azione, quando ci sar� un altro pulsante da premere
					}
		}
			...
			if (delay_fun(&time, delay_100us))	// se � passato un tempo maggiore di time	
					{
						if (HAL_GPIO_ReadPin(GPIOx, GPIO_Pin) == 1){		// ri-checko che il pulsante sia ancora pushato
							...
							if (delay_fun(&MAX_DELAY, delay_100us) && HAL_GPIO_ReadPin(GPIOx, GPIO_Pin)==0){	// se passano 10 secondi e non sto premendo pi� il pulsante, torno alla schermata RTD
							checksum = checksum_objects(WRITEOBJ, FORM, SCREEN1, MSB_FORM0, LSB_FORM0);	
							WriteObject(WRITEOBJ, FORM, SCREEN1, MSB_FORM0, LSB_FORM0, checksum);	// se dopo 10 secondi non premo pi� nulla, torno al form1 RTD
							page = 0;
							}
...
// Funzione per capire dopo quanto tempo si va in overflow e quindi si scatena un interrupt. Questa funzione � utilizzata per la delay_fun
...
uint32_t ReturnTime_100us(void) {
	...
	uint32_t time = counter;	// counter � inizializzate a 0. Ad ogni interrupt che si verifica, poi, si incrementa perch� l'ARR � andato in overflow. 
						// Il contatore, infatti, � incrementato ogni 100 us
	return time;
